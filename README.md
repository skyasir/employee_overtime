...
- **Break Time**, **Maximum Overtime Hours/Day**,
...
- **Gross Pay Based** — `(gross / days_in_month) / std`, using
...
- *OT Creation* → daily scheduler job
  (`employee_overtime.overtime.run_daily_overtime_creation`, registered under
  `scheduler_events` in hooks.py). The `after_insert` per-checkin creator
  (`create_overtime_from_checkin`) ships disabled as an alternative.
