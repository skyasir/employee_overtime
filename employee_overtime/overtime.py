...
def _create_overtime_from_checkin(doc):
    ...
    hourly = get_hourly_rate(doc.employee, std, settings, d)
...
# Rate helpers
# ---------------------------------------------------------------------------
def get_hourly_rate(employee, day_hrs, settings, d):
    ...
    days_div = frappe.utils.get_last_day(d).day
...
# ---------------------------------------------------------------------------
# Manual backfill: pull overtime for a date range (Overtime Setting button)
# ---------------------------------------------------------------------------
@frappe.whitelist()
def pull_overtime_for_range(from_date, to_date):
    """Backfill draft Employee Overtime for every day in [from_date, to_date].

    Mirrors the daily scheduler job's whole-day logic (session summing,
    category rules, grace period) so a manual backfill produces the same records
    the scheduler would. Rerun-safe: a day that already has an OT record for an
    employee is skipped. Returns the number of records created.

    Triggered from Overtime Setting: tick "Pull Records", set the dates, save.
    """
    roles = set(frappe.get_roles(frappe.session.user))
    if not ({"HR Manager", "System Manager"} & roles):
        frappe.throw("Only HR Manager or System Manager can pull overtime.")
    ...
    cfg = {
        ...
        "max_ot": settings.maximum_overtime_hours_allowed,
        "max_span": settings.maximum_shift_span_hours or MAX_SPAN,
        "base_mult": settings.standard_multiplier or 1.0,
        "apply_ph": settings.applicable_for_public_holiday,
        "apply_we": settings.applicable_for_weekend,
        "ph_mult": settings.public_holiday_multiplier,
        "we_mult": settings.weekend_multiplier,
        "rules": settings.get("category_rules") or [],
    }
    ...
    day = from_date
    while day <= to_date:
        created += _pull_overtime_for_day(day, cfg, issues)
        frappe.db.commit()
        day = getdate(frappe.utils.add_days(day, 1))

    if issues:
        frappe.log_error(
            message="\n".join(issues),
            title=f"OT Pull anomalies {from_date} to {to_date}",
        )
    return created


def _pull_overtime_for_day(day, cfg, issues):
    """Create OT for one calendar day across all employees. Returns count."""
    day = getdate(day)
    is_ph = bool(frappe.db.exists("Holiday", {"holiday_date": day}))
    is_we = day.weekday() >= 5

    win_start = frappe.utils.get_datetime("%s 00:00:00" % day)
    win_end = frappe.utils.get_datetime(
        "%s 12:00:00" % frappe.utils.add_days(day, 1)
    )
    ...
    for employee, punches in by_emp.items():
        ...
        if frappe.db.exists(
            "Employee Overtime",
            {"employee_id": employee, "date": day, "docstatus": ["<", 2]},
        ):
            continue

        emp = frappe.db.get_value(
            "Employee", employee,
            ["custom_ot_eligible", "custom_ot_category"], as_dict=True,
        ) or {}
        if not emp.get("custom_ot_eligible"):
            continue
        ...
        # Category holiday policy: some categories do not earn OT on a holiday
        # or weekly off.
        if category and (is_ph or is_we):
            allow = frappe.db.get_value(
                "Overtime Category", category, "allow_overtime_on_holiday"
            ) or 0
            if not allow:
                continue

        # Pair punches into sessions and sum worked time.
        worked = 0.0
        pending_in = None
        first_in = first_in_name = None
        last_out = last_out_name = None
        shift = None
        ...
        for p in punches:
            ...
            if p.log_type == "IN":
                if pending_in:
                    issues.append(
                        "%s %s: IN at %s had no matching OUT (overwritten by IN at %s)"
                        % (day, employee, pending_in.t, p.t)
                    )
                pending_in = p
                if not first_in:
                    first_in, first_in_name = p.t, p.name

            elif p.log_type == "OUT" and pending_in:
                span = time_diff_in_hours(p.t, pending_in.t)
                if span > cfg["max_span"]:
                    issues.append(
                        "%s %s: session %s -> %s = %.2f hrs exceeds max span, skipped"
                        % (day, employee, pending_in.t, p.t, span)
                    )
                elif span > 0:
                    worked += span
                    last_out, last_out_name = p.t, p.name
                pending_in = None

        if pending_in:
            issues.append(
                "%s %s: IN at %s had no OUT by end of window (missed punch)"
                % (day, employee, pending_in.t)
            )
        ...
        if shift:
            std = frappe.db.get_value(
                "Shift Type", shift, "custom_standard_working_hours"
            ) or 0
        if not std:
            issues.append(
                "%s %s: no standard hours for shift '%s', skipped"
                % (day, employee, shift)
            )
            continue

        # Category rule: standard hours + break deduction for this shift length.
        emp_std = std
        emp_break = cfg["break_hrs"]
        if category:
            for rule in cfg["rules"]:
                ...
                if abs((rule.shift_hours or 0) - std) < HOURS_TOLERANCE:
                    ...
                    emp_break = (rule.break_hours or cfg["break_hrs"]) if rule.deduct_break else 0
                    break
        ...
        # Grace gate.
        if ot_hours <= 0 or ot_hours < cfg["grace"]:
            continue

        if cfg["max_ot"] and ot_hours > cfg["max_ot"]:
            ot_hours = cfg["max_ot"]

        # Rate: latest assignment effective on/before this day (gross, else base).
        gross = frappe.db.get_value(
            "Salary Structure Assignment",
            {"employee": employee, "docstatus": 1, "from_date": ["<=", day]},
            "custom_gross_pay", order_by="from_date desc",
        ) or 0
        if not gross:
            gross = frappe.db.get_value(
                "Salary Structure Assignment",
        ...
        hourly = (gross / frappe.utils.get_last_day(day).day) / emp_std if (emp_std and gross) else 0

        multiplier = cfg["base_mult"]
        if cfg["apply_ph"] and is_ph:
            multiplier = cfg["ph_mult"] or multiplier
        elif cfg["apply_we"] and is_we:
            multiplier = cfg["we_mult"] or multiplier
...
# ---------------------------------------------------------------------------
# Daily scheduler job (runs from the app, not a desk Server Script)
# ---------------------------------------------------------------------------
def run_daily_overtime_creation():
    """Create draft Employee Overtime for the previous day's punches.

    Registered under ``scheduler_events`` in hooks.py and run daily. Uses the
    same whole-day logic as the manual pull (session summing, category rules,
    grace period), so the OT records match what the earlier daily Server Script
    produced - but the logic lives in the app and nothing is created in the
    desk's Server Script list.
    """
    target = getdate(frappe.utils.add_days(frappe.utils.nowdate(), -1))
    ...
    issues = []
    created = _pull_overtime_for_day(target, cfg, issues)
    frappe.db.commit()
